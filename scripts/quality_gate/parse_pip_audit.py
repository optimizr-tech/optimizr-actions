"""Parse pip-audit JSON into a normalized security metric."""
...
def parse(path: Path, scope: Scope) -> SecurityMetric:
    ...
    if not isinstance(dependencies, list):
        raise ValueError(f"{path}: missing 'dependencies' list")
    high = 0
    for dependency in dependencies:
        if isinstance(dependency, dict):
            findings = dependency.get("vulns", []) or []
            high += sum(isinstance(finding, dict) for finding in findings)
