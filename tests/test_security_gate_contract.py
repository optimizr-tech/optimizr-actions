...
class SecurityGateContractTests(unittest.TestCase):
    ...
    def test_single_service_deploy_gates_filesystem_and_image_before_rollout(self) -> None:
        ...
        sync = content.index("Backup and sync files")
        build = content.index("Build image")
        image = content.index("Security gate (images)")
        deploy = content.index("- name: Deploy")
    ...
    def test_monorepo_deploy_gates_filesystem_and_images_before_rollout(self) -> None:
        ...
        self.assertLess(filesystem, sync)
        self.assertLess(build, image)
        self.assertLess(image, rollout)
        self.assertIn("SECURITY_IMAGE_SERVICES", content)
        self.assertIn("No Compose images available for required security scan", content)
        self.assertIn("Upload security evidence", content)
