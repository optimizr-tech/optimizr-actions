...
- `pyproject.toml` with one of `uv.lock`, `poetry.lock`, or hash-locked `requirements*.txt`;
- standalone hash-locked `requirements*.txt` using exact `==` pins and SHA-256 hashes;
...
The native package manager performs an immutable/frozen lock check. A missing tool, network failure, stale lockfile, stale or missing advisory database, malformed policy, denied license, or unexcepted High/Critical advisory fails closed.
...
Policy exceptions are exact by kind, advisory/license identifier and package. Every exception requires owner, statement and ISO expiry. Evidence records lockfile and policy hashes, direct/development/transitive classification counts, Trivy/database identity, repository/SHA, the Trivy report hash, suppressed findings and blocking findings; it does not serialize credentials or package-manager environments.
