...
Exclusions must be repository-relative glob patterns and may not contain traversal. Keep them narrow and explain them in the consumer PR. The gate retains ShellCheck text, actionlint path/line diagnostics without source snippets, composite metadata failures, discovered files, versions, and command exit status. It receives no secrets and uses `contents: read`.
